...
def CIFAR_loader(path, batch_size, num_workers=4, pin_memory=False, normalize=None):
    dl, ds = {}, {}
    # Normalize data sets
    # if per_task_norm:
    #     tsforms = {
    ...
    ds['train'] = datasets.ImageFolder(path,
                                       tsforms['train'])
    ds['test'] = datasets.ImageFolder(path,
                                      tsforms['test'])
    dl['train'] = torch.utils.data.DataLoader(ds['train'], batch_size=batch_size, shuffle=True,
                                              num_workers=num_workers)
    dl['test'] = torch.utils.data.DataLoader(ds['test'], batch_size=batch_size, shuffle=False,
                                             num_workers=num_workers)
