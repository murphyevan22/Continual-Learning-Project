...
class SparsePruner(object):
    ...
    def _pruning_mask(self, weights, mask, layer_name, pruning_ratio):
        ...
        print("Cutoff Rank: ", cutoff_rank)
        #cutoff_value = abs_tensor.view(-1).cpu().kthvalue(cutoff_rank)[0].item()
        cutoff_value = abs_tensor.view(-1).cpu().kthvalue(cutoff_rank)[0].cuda()
        print("Cutoff (CUDA)", cutoff_value)
        cutoff_value = abs_tensor.view(-1).cpu().kthvalue(cutoff_rank)[0].item()
        print("Cutoff (ITEM)", cutoff_value)
        ...
        remove_mask = weights.abs().le(cutoff_value) * \
                      mask.eq(self.current_dataset_idx)
            ...
            for name, module in self.model.named_modules():
                if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                    if 'classifiers' in name:
                        continue
                    mask = self._pruning_mask(module.weight.data, self.masks[name], name,
                                              pruning_ratio=curr_pruning_ratio)
