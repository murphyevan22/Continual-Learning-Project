...
class ResNet18(ModifiedVGG16):
    """ResNet-18."""

    def __init__(self, make_model=True):
        super(ResNet18, self).__init__(make_model=False)
        if make_model:
            self.make_model()
    ...
    def make_model(self):
        ...
        for name, module in resnet.named_children():
            ...
            #     module == nn.Linear()
            #     self.shared.add_module(name, module)
        # self.shared.fc = nn.Linear(512, num_outputs)
        # Add the default imagenet classifier.
        # self.datasets.append('imagenet')
        # self.classifiers.append(resnet.fc)
        print(self.shared)
    ...
    def add_dataset(self, dataset, num_outputs):
        ...
        if dataset not in self.datasets:
            ...
            if not self.classifier:
                ...
                self.increment_classes(num_outputs)

# Based on implementation: https://github.com/ngailapdi/LWF/blob/master/model.py
    def increment_classes(self, num_classes):
        """Add new classes to the final fully connected layer"""
        ...
        #self.classifier = nn.Linear(in_features, new_out_features, bias=False)
        self.classifier = nn.Linear(in_features, new_out_features)

        #kaiming_normal_init(self.classifier.weight)
        # Restore existing weight values to classifier
        # Leaves new portion randomly initialized
